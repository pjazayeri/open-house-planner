...
- **[HARD]** Build a DB service for listing / open-house data (user-agnostic catalog in Neon). Today listings come from a per-user Redfin CSV blob that's re-parsed on every load; there's no shared, queryable, historical store of listings or their open-house times. Move that into Neon so listing data is deduped by address, queryable, and accumulates open-house history across weekends — decoupled from any one user's CSV.
  ...
  - ✅ **Option A read-path DONE (2026-05-24):** `api/listings` GET (auth-gated, soonest upcoming OH per address; + dev middleware) and `overlayOpenHouses()` in `useListings` refresh stale CSV open-house times from the catalog by address. So uploads no longer need to happen weekly. *(Pending in-app confirmation by a signed-in user.)*
  - **Remaining:**
    - ~~Coverage~~ **→ FIXED (2026-05-25):** Redfin caps each query at ~350 rows, so the cron now pulls **all ~1000+ SF listings** via adaptive price-band slicing (recursively split any band hitting the cap). Was the cause of "no upcoming open houses" — the top-350 "recommended" slice almost never contained a user's favorites. Catalog: 354→1032 listings.
    - **[MEDIUM] Option B — in-app favorites.** Decision: a **new `favorite` set** = the user's "my listings" universe (distinct from the existing star/priority = ranking, and hide). A favorite button on any catalog listing adds to it; CSV import also feeds it. Needs: `favoriteIds` in `user_state`, a Browse-from-catalog view (`api/listings` returning full listing rows, not just OH times), a favorite toggle, and `useListings` sourcing its universe from favorites∪CSV.
    - **[MEDIUM] Admin city-control portal.** NOTE: data volume is a non-issue on Neon free (open_houses ≈ 2 MB/city/year; ~250 city-years to fill 0.5 GB). Real value = curating *which* metros to ingest. Needs: `ingest_regions(slug, name, region_id, region_type, enabled)` table; cron loops *enabled* rows; admin-gated page (allowlist the owner uid) to toggle/add cities; a `last_seen`-based prune for `listings` hygiene.
    - **[MEDIUM]** Stage 2 of the data-model migration: once the client fully sources listings from the catalog, re-key user state by `address_key` and **delete `relinkIds.ts` + the snapshot bandaid**.
    - Add unit tests for the cron's CSV parse + address-key upsert + open-house append/dedupe logic.
