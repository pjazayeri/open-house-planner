...
function DetailPanel({ listing, result, downPct, ratePct, termYears, oppReturnPct, taxRatePct, appreciationPct, includePrincipal, rentOverride, onSetRentOverride, holdYears, setHoldYears, buyerClosingPct, setBuyerClosingPct, sellerCostPct, setSellerCostPct, rentInflationPct, setRentInflationPct }: DetailProps) {
  ...
  const tipTaxSavings = [
    ...
    `Phases out 30% above $500k MAGI (joint) / $250k`,
    `(single). Floor: $10k minimum regardless.`,
    ``,
    `If your state income tax is ~$30k, you have ~$10k`,
    `of SALT headroom for property tax.`,
    ``,
    `Only applies if total itemized deductions`,
    `(SALT ≤$40k + ~${fmtDollar(annualInterest)}/yr interest + charitable)`,
    `exceed your standard deduction (~$15.7k single /`,
    `~$31.5k married in 2025).`,
    ``,
