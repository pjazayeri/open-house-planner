...
function DetailPanel({ listing, result, downPct, ratePct, termYears, oppReturnPct, taxRatePct, appreciationPct, includePrincipal, rentOverride, onSetRentOverride, holdYears, setHoldYears, buyerClosingPct, setBuyerClosingPct, sellerCostPct, setSellerCostPct, rentInflationPct, setRentInflationPct }: DetailProps) {
  ...
  const tipTaxSavings = [
    ...
    ``,
    `SALT cap: $40k base under OBBBA (signed Jul 2025),`,
    `but phases out 30% above $250k MAGI (single) /`,
    `$500k (joint). Floor: $10k minimum.`,
    ``,
    `Example: $300k MAGI single →`,
    `  $40k − 30%×($300k−$250k) = $40k−$15k = $25k cap`,
    `  CA income tax ~$30k already exceeds $25k cap →`,
    `  property tax adds no incremental SALT benefit.`,
    ``,
    `CA state income tax counts as SALT. Only mortgage`,
    `interest (this row) is tracked separately.`,
    ``,
    `Itemizing beats standard deduction (~$15.7k single`,
    `/ ~$31.5k married 2025) if SALT + ~${fmtDollar(annualInterest)}/yr`,
    `interest + charitable > standard deduction.`,
    ``,
