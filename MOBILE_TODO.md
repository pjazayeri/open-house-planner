...
- [ ] **Leaving the map during its zoom animation throws a Leaflet exception (and can eat the tap)** — load `/#demo` (Browse map fits bounds with an animated zoom) and within ~1 s tap Data or Analytics in the bottom bar → console: `TypeError: Cannot read properties of undefined (reading '_leaflet_pos')` from `_onZoomTransitionEnd`; on one run the first tap did not switch pages (second tap did). Cause: App unmounts `MapView` on page change while Leaflet's zoom transition is in flight → expected: either keep the map mounted across pages (hide with CSS — also preserves the user's pan/zoom) or pass `zoomAnimation={false}` / cancel the animation on unmount (`map.stop()` in cleanup). Test: reload, tap a bottom tab within 1 s, ten times → zero console exceptions and every tap switches on the first try.
