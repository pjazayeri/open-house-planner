...
- [ ] **Planner map: clusters hide the tour-stop numbers along the route** — Open Houses → Map at 390px, default zoom: the dashed route runs between ×2 / ×5 / ×8 bubbles, so the stop order the route exists to show is invisible until zoomed in → expected: on the planner (route) map either don't cluster (≤ ~40 stops is manageable) or label bubbles with the stop range they contain ("3–7") and keep the next unvisited stop as an individual pin. Test: on the planner map at the fit zoom, the first three stops of the route are readable as numbers without zooming.
