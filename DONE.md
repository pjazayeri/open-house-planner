...
- **Design & Architecture page (`/#design`) + `docs/DESIGN.md`** *(shipped 2026-05-24)* — In-app high-level design overview (architecture layers, data-flow, decision cards) linking to the full `docs/DESIGN.md`. "Design" nav tab visible to all (incl. guest/demo) as a showcase. *Verify:* open `/#design`, confirm it renders and the doc link works.
- **Admin observability dashboard (`/#admin`) + `api/admin-stats`** *(shipped 2026-05-24)* — Owner-only (ADMIN_UIDS) live data-volume across Neon (db/table sizes vs 0.5GB), Vercel Blob (bytes/count), Firebase (users), + catalog health + metered-dep reference cards. Endpoint verified live + gated (401 unauth). *Verify:* sign in as owner, open `/#admin`, confirm real numbers render.
